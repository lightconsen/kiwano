#!/usr/bin/env node
// Build kiwanod and stage it for Tauri's `externalBin`.
//
// Tauri resolves the config entry `binaries/kiwanod` to
// app/src-tauri/binaries/kiwanod-<target-triple>[.exe] — the triple the
...
const built = join(ROOT, "target", ...(requested ? [triple] : []), profile, `kiwanod${exe}`);
const staged = join(STAGE, `kiwanod-${triple}${exe}`);
...
rmSync(join(STAGE, `kiwanod${exe}`), { force: true });
