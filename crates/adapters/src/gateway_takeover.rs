...
//!
//! Cline is the exception this file's name does not cover: its selector names a
//! provider *id* rather than an entry of ours, so the takeover replaces the slot
//! that id names. See [`upsert_cline_gateway`] for why that is not a
//! preference.
