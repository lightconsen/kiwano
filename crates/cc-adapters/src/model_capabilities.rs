...
// Source: src-tauri/src/model_capabilities.rs
// Copied on 2026-09-07. Modified for Kiwano (visibility relaxed from
// `pub(crate)` to `pub` for cross-crate use, and the one-million-context
// marker inlined from `claude_desktop_config`).

use serde_json::Value;

/// Suffix cc-switch/Kiwano append to model IDs that opted into the 1M context
/// window. Inlined from cc-switch `claude_desktop_config::ONE_M_CONTEXT_MARKER`.
...
pub fn image_input_capability_from_settings(
    settings: &Value,
    model: &str,
    use_confirmed_registry: bool,
) -> ImageInputCapability {
...
pub fn image_input_capability_from_modalities(
    model: &str,
