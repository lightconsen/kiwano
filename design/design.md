...
## 9. 架构决策：Agent 默认接管本地网关（2026-09-07；归因方案 09-07 修订；接管入口 09-19 迁往 Apps）
...
- **代价/配套**：网关常驻守护（GUI 关了代理还在）；每 Agent「接管/还原」开关 + 原配置备份（Agent 自己的设置弹窗里，还原=逃生门）；Aider/Continue 等不接管
- **UI 落点**：设置 → 本地网关只剩一行汇总（已接管数 / 内置总数 ＋「在应用中管理」）；逐个体操在 Apps → 该 Agent 的 tab → 齿轮弹窗（占位 Key、被改写的文件、关闭接管）。09-19 之前这些行逐条列在设置页，agent 增多后随注册表一起变长；首屏 footer 文案「Agent 已接管至本地网关，切换仅改路由」；admin 控制端口 :8310 不在 UI 露出
